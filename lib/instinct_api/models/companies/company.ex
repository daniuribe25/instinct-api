defmodule InstinctApi.Models.Companies.Company do
  use Ecto.Schema
  ...
  def validate_email_unique(changeset, field) do
    case get_field(changeset, field) |> InstinctApi.Services.Users.get_by_email() do
      %InstinctApi.Models.Users.User{} = _ -> add_error(changeset, :email, "already registered as job seeker")
      _ -> changeset
    end
  end

  def validate_phone_unique(changeset, field) do
    case get_field(changeset, field) |> InstinctApi.Services.Users.get_by_phone() do
      %InstinctApi.Models.Users.User{} = _ -> add_error(changeset, :phone, "already registered as job seeker")
      _ -> changeset
    end
  end
